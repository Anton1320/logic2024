...
тогда, когда $\ulcorner x\urcorner \in \text{Tr}_\text{ФА}$. \end{thm}
\begin{proof}
...
Пусть $\varphi$ таково, что $\llbracket\varphi(x)\rrbracket = \text{И}$ при $\ulcorner x\urcorner \in \text{Tr}$.
Тогда $\vdash\varphi(x)$, если $\ulcorner x\urcorner \in \text{Tr}$ и $\vdash\neg\varphi(x)$, если $\ulcorner x\urcorner \notin\text{Tr}$.
